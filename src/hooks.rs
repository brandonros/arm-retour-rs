...
pub fn enable_hook(ori_fn_address: usize, patch_fn_bytes: &Vec<u8>) {
    patch_function_memory((ori_fn_address - 1) as *const c_void as *mut u8, &patch_fn_bytes);
}

pub fn disable_hook(ori_fn_address: usize, ori_fn_bytes: &Vec<u8>) {
    patch_function_memory((ori_fn_address - 1) as *const c_void as *mut u8, &ori_fn_bytes);
}

pub fn build_hook(ori_fn_address: *const c_void, hook_fn_address: *const c_void, ori_fn_bytes_length: usize) -> Hook {
...
fn patch_function_memory(target_func: *mut u8, patch_instructions: &[u8]) {
    ...
    unsafe {
        std::ptr::copy_nonoverlapping(
            patch_instructions.as_ptr(),
            target_func,
            patch_instructions.len(),
        );
    }
...
pub fn backup_original_bytes(address: *const c_void, length: usize) -> Vec<u8> {
